...
| `build_parent_counts()` | 구현·기능 검증 완료 | 각 커밋의 직접 부모 수를 별도 정수 표로 준비하는 원리 |
| 기본 LOG의 순서 구성 | `topological_order()` 구현·기능 검증 완료 | 모든 부모를 먼저 처리하는 위상 정렬 |
...
**근거: Subject.txt 4장 LOG. 현재 `topological_order()` 구현이 따르는 원리를 설명한다. 아래 실행 표의 후보 선택 순서는 예시이며, 현재 코드는 ready의 마지막 항목을 꺼낸다.**
...
#### 12.4.1 삭제하지 않는 큐에서 현재 ID를 읽기

PATH의 거리 탐색에서는 리스트 맨 앞을 `pop(0)`으로 삭제하는 대신, 다음에 읽을 인덱스를 `read_position`에 보관할 수 있다. 시작 커밋이 h0이면 초기값은 다음과 같다.
...
`waiting[read_position]`은 현재 읽을 ID를 뜻한다. 위 상태에서 `waiting[0]`의 값은 문자열 `"h0"`다. 그 값을 `current_id`에 보관한 다음 `read_position`을 1 증가시키면, h0는 처리 대상으로 선택되었고 다음에는 인덱스 1을 읽겠다는 뜻이 된다.
...
두 줄의 실행 전후 값은 다음과 같다.

| 시점 | `waiting` | `read_position` | `current_id` |
| --- | --- | --- | --- |
| 실행 전 | `["h0"]` | `0` | 아직 없음 |
| 첫 줄 실행 후 | `["h0"]` | `0` | `"h0"` |
| 둘째 줄 실행 후 | `["h0"]` | `1` | `"h0"` |

`read_position`이 바뀌어도 `current_id`의 값이 h1로 자동 변경되는 것은 아니다. `current_id`에는 이미 리스트에서 읽어 온 문자열 `"h0"`가 들어 있다. 다음 `while` 반복이 시작되어 다시 첫 줄을 실행할 때 새로운 현재 ID를 읽는다.

현재 h0의 이웃 h1과 h2를 새로 발견해 `waiting` 뒤에 추가하면 다음 상태가 된다.
...
다음 반복에서는 `1 < 3`이므로 계속 진행하고, `waiting[1]`인 `"h1"`을 읽는다. 이런 식으로 이미 읽은 ID는 리스트 앞에 그대로 남아 있지만, 읽는 위치가 오른쪽으로 이동한다.

| 반복 | 반복 시작의 `waiting` | 읽는 위치 | 이번 `current_id` | 읽은 뒤 위치 |
| --- | --- | --- | --- | --- |
| 1 | `["h0"]` | 0 | `"h0"` | 1 |
| 2 | `["h0", "h1", "h2"]` | 1 | `"h1"` | 2 |
| 3 | `["h0", "h1", "h2", "h3"]` | 2 | `"h2"` | 3 |
| 4 | `["h0", "h1", "h2", "h3"]` | 3 | `"h3"` | 4 |

마지막에는 `read_position == len(waiting) == 4`가 되어 반복이 끝난다. `waiting`이 비어서 끝나는 것이 아니다.

순서도 중요하다. 위치를 먼저 증가시키고 리스트를 읽으면 첫 반복부터 인덱스 1을 읽으려 하므로 시작점 h0를 건너뛰고, 초기 목록에는 인덱스 1이 없어 오류까지 발생한다. 따라서 **현재 위치에서 먼저 읽고, 그 다음 읽을 위치를 증가**시킨다.

#### 12.4.2 현재 ID의 이웃을 확인하고 새 이웃을 등록하기

현재 ID를 읽은 뒤에는 `neighbors` 딕셔너리에서 그 ID의 이웃 목록을 가져온다. 현재 ID가 h0이고 이웃 표가 다음과 같다고 하자.
...
`neighbors[current_id]`는 `neighbors["h0"]`와 같으므로 값은 `["h1", "h2"]`다. 이 목록을 순회할 때 `neighbor_id`에는 첫 번째 반복에서 문자열 `"h1"`, 두 번째 반복에서 문자열 `"h2"`가 들어간다. 이것이 “현재 ID의 이웃들을 하나씩 확인한다”는 뜻이다.

```python
for neighbor_id in neighbors[current_id]:
    # 첫 반복의 neighbor_id는 "h1"
    # 둘째 반복의 neighbor_id는 "h2"
```

하지만 이웃을 확인했다는 이유만으로 무조건 거리와 대기 목록에 다시 넣으면 안 된다. 무방향 관계이므로 h1의 이웃에는 다시 h0가 있다. 이미 발견한 h0를 또 등록하면 h0와 h1 사이를 계속 왕복할 수 있다.

`distance` 딕셔너리에 ID가 키로 있다는 것은 그 ID를 이미 발견했다는 뜻으로 사용한다.

```text
neighbor_id가 distance에 있음     → 이미 발견함 → 아무것도 하지 않음
neighbor_id가 distance에 없음     → 처음 발견함 → 거리 기록 후 waiting에 추가
```

현재 ID에서 이웃 ID까지는 간선 하나를 이동하므로 새 거리는 현재 거리보다 1 크다.

```python
distance[neighbor_id] = distance[current_id] + 1
```

거리 기록과 대기 목록 추가를 한 흐름으로 쓰면 다음과 같다.
...
이 네 줄은 현재 ID를 읽는 `while` 반복 안에 들어가며, 거리 기록과 `append()` 두 줄은 “처음 발견한 경우”를 뜻하는 `if` 안에 들어간다.

h0에서 탐색을 시작한 첫 반복의 상태 변화는 다음과 같다.

| 처리 시점 | `neighbor_id` | 이미 `distance`에 있는가? | 기록할 거리 | 처리 후 `waiting` | 처리 후 `distance` |
| --- | --- | --- | --- | --- | --- |
| 시작 | 아직 없음 | h0만 있음 | h0 = 0 | `["h0"]` | `{"h0": 0}` |
| h1 확인 | `"h1"` | 아니요 | 0 + 1 = 1 | `["h0", "h1"]` | `{"h0": 0, "h1": 1}` |
| h2 확인 | `"h2"` | 아니요 | 0 + 1 = 1 | `["h0", "h1", "h2"]` | `{"h0": 0, "h1": 1, "h2": 1}` |

다음 `while` 반복에서 h1을 처리할 때 이웃이 `["h0", "h3"]`이라면 다음과 같이 판단한다.

- h0는 이미 `distance`에 있으므로 건너뛴다.
- h3는 처음 발견했으므로 `distance["h3"] = distance["h1"] + 1`, 즉 2를 기록하고 대기 목록에 추가한다.

그 결과는 다음과 같다.

```python
waiting == ["h0", "h1", "h2", "h3"]
distance == {"h0": 0, "h1": 1, "h2": 1, "h3": 2}
```

여기서 `waiting`은 발견·처리 순서를 관리하고, `distance`는 최소 거리와 발견 여부를 관리한다. 두 자료구조는 같은 ID를 담을 수 있지만 역할이 다르다.
...
#### 같은 ID 하나가 든 경로와 문자열의 글자 목록을 구분하기

`start_hash`가 문자열 `"h0"`일 때 `list(start_hash)`와 `[start_hash]`는 다른 결과를 만든다.

| 표현 | Python이 하는 일 | 결과 | 결과 타입 |
| --- | --- | --- | --- |
| `list(start_hash)` | 문자열을 순회해 각 글자를 원소로 넣음 | `["h", "0"]` | `list[str]`이지만 각 원소는 ID 전체가 아니라 한 글자 |
| `[start_hash]` | 문자열 변수 하나를 리스트의 원소로 넣음 | `["h0"]` | ID 문자열 하나가 든 `list[str]` |

PATH가 반환해야 하는 것은 글자들의 목록이 아니라 방문한 **커밋 ID들의 목록**이다. 시작과 도착이 모두 h0이면 커밋 하나를 방문한 경로이므로 `[start_hash]`, 즉 `["h0"]`를 반환해야 한다.
...
둘 다 겉으로는 `list[str]`이지만 문자열 원소가 무엇을 뜻하는지가 다르다. 타입 표기만 확인하지 말고 각 원소가 커밋 ID 전체인지도 확인해야 한다.
...
#### 13.5.1 `best_suffix`와 `next_step`에 최선의 다음 ID 기록하기

두 딕셔너리는 서로 다른 값을 저장한다.

| 이름 | 타입 | 키 → 값의 예 | 역할 |
| --- | --- | --- | --- |
| `best_suffix` | `dict[str, str]` | `"h1" → "h1->h3"` | 현재 ID부터 목표까지 선택한 전체 경로 문자열 |
| `next_step` | `dict[str, str]` | `"h1" → "h3"` | 그 전체 경로를 따라갈 때 바로 다음에 이동할 ID |

목표가 h3이면 처음에는 다음 상태다.
...
h3은 이미 목표이므로 남은 전체 경로 문자열이 `"h3"`다. 하지만 h3 다음에 이동할 ID는 없으므로 `next_step`에는 h3의 항목을 넣지 않는다.

거리 표는 목표에서 BFS로 ID를 발견한 순서대로 값이 추가되었다. Python 3.10의 딕셔너리는 삽입 순서를 유지하므로 `distance`를 순회하면 거리 0인 목표, 거리 1인 노드들, 거리 2인 노드들 순으로 처리된다. 현재 노드보다 거리가 1 작은 이웃의 `best_suffix`가 먼저 준비되는 이유다.

현재 ID가 h0이고 목표 h3까지의 거리 정보가 다음과 같다고 하자.
...
현재 거리보다 정확히 1 작은 이웃만 최단 경로 후보다. 각 후보의 전체 경로 문자열은 현재 ID, 구분자, 그 이웃의 최선 suffix를 이어서 만든다.

```text
h1 후보 = "h0" + "->" + best_suffix["h1"]
        = "h0->h1->h3"

h2 후보 = "h0" + "->" + best_suffix["h2"]
        = "h0->h2->h3"
```

처음 만난 후보는 비교 대상이 아직 없으므로 바로 기록한다. 그다음 후보부터는 지금까지 기록한 문자열보다 작을 때만 두 딕셔너리의 값을 함께 교체한다.

```text
현재 ID가 best_suffix에 아직 없음
또는 새 후보 문자열이 기존 best_suffix[현재 ID]보다 작음
    → best_suffix[현재 ID]를 새 전체 문자열로 교체
    → next_step[현재 ID]를 그 후보 이웃 ID로 교체
```

`or` 조건을 사용할 경우 왼쪽 조건부터 평가된다. 현재 ID가 아직 키로 없으면 왼쪽이 참이므로 오른쪽의 `best_suffix[current_id]` 조회는 실행되지 않는다. 따라서 첫 후보에서 존재하지 않는 키를 읽는 오류를 피하면서 값을 처음 등록할 수 있다.

이웃이 h2, h1 순으로 들어와도 결과는 같다.

| 확인 순서 | 새 후보 | 처리 |
| --- | --- | --- |
| h2 먼저 | `h0->h2->h3` | 첫 후보이므로 임시 최선으로 기록 |
| h1 다음 | `h0->h1->h3` | 기존 문자열보다 작으므로 h1 후보로 교체 |

최종 상태는 다음과 같다.

```python
best_suffix["h0"] == "h0->h1->h3"
next_step["h0"] == "h1"
```

따라서 이웃 나열 순서가 정답을 결정하지 않는다. 모든 유효 후보의 **전체 경로 문자열**을 직접 비교한 결과가 정답을 결정한다.
...
현재 기본 LOG의 순서는 `topological_order()`가 ID 리스트로 반환한다. PATH 함수의 정확한 이름과 내부 반환 계약은 구현 단계에서 정한다. 명령의 화면 결과는 명세에 맞추어야 하지만 내부 표현까지 모두 명세가 고정한 것은 아니다.
...
#### 15.3.1 format_log로 출력할 문자열 준비하기

`main.py`에는 계산된 순서를 받아 출력용 문자열로 만드는 독립 함수를 준비한다. 이 함수 이름과 문자열 형태는 구현 선택이다. 명세는 hash·author·timestamp·message를 식별할 수 있도록 요구한다.
...
두 매개변수의 역할이 다르다. `commits`는 실제 정보를 찾는 저장소이고, `ordered_ids`는 그 정보를 보여 줄 순서다. 순회할 대상은 ordered_ids다. 각 ID로 commits에서 객체를 조회하고, 객체의 네 필드를 문자열에 넣는다.

예를 들어 선택할 수 있는 출력 형태는 다음과 같다. 커밋 하나당 첫 줄에 ID·작성자·시각을, 다음 줄에 메시지를 넣는다.

```text
commit CI_0 (Alice, 2026-09-14 10:30:00)
Initial commit
commit CI_1 (Alice, 2026-09-14 10:31:00)
Add login feature
```

문자열 안에 값을 넣을 때는 f-string을 사용할 수 있다.

```python
name = "Alice"
label = f"작성자: {name}"
# label은 "작성자: Alice"라는 문자열이다.
```

중괄호 안의 표현식을 계산한 값이 문자열에 들어간다. 실제 함수에서는 조회한 커밋 객체의 속성들을 사용할 수 있다. datetime 객체도 f-string에 넣어 문자열로 표현할 수 있으며, 명세가 시각의 세부 출력 형식이나 소수점 자릿수를 고정한 것은 아니다.

여러 줄을 모을 때는 문자열 리스트를 준비하고, 반복문이 끝난 뒤 줄바꿈 문자 `\n`으로 연결할 수 있다.
...
이 text에는 두 줄을 나타내는 문자열이 들어 있다. 반환 전에 직접 print하는 것과는 다르다. 함수는 문자열을 반환하고, 나중에 호출한 쪽에서 그 문자열을 출력하도록 둔다.

이 단계의 반환 계약은 빈 ID 목록이면 빈 문자열 `""`을 반환하는 것으로 정한다. 이는 출력 형식의 구현 선택이며 명세가 특정한 빈 결과 문구를 요구한 것은 아니다.

형식을 만드는 함수는 전달받은 순서를 그대로 따른다. 여기서 다시 정렬하거나 집합으로 바꾸지 않는다. 이렇게 하면 기본 LOG가 계산한 순서를 유지할 수 있고, 이후 날짜·작성자 정렬이 만든 순서에도 같은 형식을 적용할 수 있다.

전체 연결은 `topological_order()`로 ID 순서 계산 → ID로 Commit 객체 조회 → `format_log()`에서 문자열 구성 → CLI에서 출력의 흐름이다. 객체 조회는 위 format_log 함수 안에서 수행하며, 명령 파싱과 REPL 연결은 CHECK.md 11단계에서 다룬다.
...
실제 경로에서 다음 노드로 선택할 수 있는 후보는 부모뿐이다. 하지만 현재 코드에서는 **실제 이동 후보와, 거리를 계산하기 위해 BFS에서 읽는 관계를 구분**해야 한다.

#### 현재 코드가 도착점에서 BFS하는 이유

`find_shortest_path()`는 다음 호출로 거리를 구한다.

```python
distance = build_distance_map(commits, end_hash)
```

즉, 함수에 주어진 출발점이 아니라 **도착점 `end_hash`**부터 BFS한다. 현재 무방향 그래프에서는 도착점에서 어떤 노드까지의 거리와 그 노드에서 도착점까지의 거리가 같다. 그래서 `distance[current_id]`를 “현재 노드에서 목표까지 남은 최소 간선 수”로 사용할 수 있다.

부모 방향만 허용하는 그래프에서는 이 대칭성이 사라진다. B의 부모가 A일 때 B → A는 가능해도 A → B는 불가능하다.

#### 부모 목록으로만 바꾸면 실패하는 예

다른 노드 없이 A와 B만 있고, B의 부모가 A라고 하자. 요청은 `PATH B A`이며 정답은 `B → A`다.

| 거리 계산 방법 | 시작점에서 읽는 연결 | 만들어지는 `distance` | 결과 |
| --- | --- | --- | --- |
| 도착점 A에서 부모 방향으로 BFS | A에는 부모가 없다. | `{'A': 0}` | B가 표에 없어 실제로 존재하는 경로를 놓친다. |
| 도착점 A에서 자식 방향으로 BFS | A의 자식 B를 발견한다. | `{'A': 0, 'B': 1}` | B에서 A까지 남은 거리가 1임을 알 수 있다. |

두 번째 행의 거리 표는 `dict[str, int]`다. 키 `'B'`는 커밋 ID이고, 값 `1`은 **B에서 목표 A까지 부모 방향으로 이동할 때 필요한 최소 간선 수**다. 그 값을 계산하는 BFS는 반대 방향인 A → B로 진행한 것이다.

이처럼 실제 이동 방향을 뒤집어 도착점에서 탐색하는 것을 역방향 BFS로 이해할 수 있다. 부모 방향의 `B → A`를 거리 계산에서는 자식 방향의 `A → B`로 읽는다. 간선을 뒤집어도 대응하는 경로의 간선 수는 같으므로 목표까지 남은 거리를 계산할 수 있다. 저장된 `parents`를 변경하는 작업은 아니다.

#### 현재 구조를 유지할 때 바뀌어야 할 부분

현재 `build_neighbors_map()`은 `build_distance_map()` 안과 `find_shortest_path()` 안에서 모두 사용된다. 이 함수의 결과를 일괄적으로 부모 목록으로 바꾸면 위의 두 역할을 함께 바꾸게 되므로 충분하지 않다.

| 역할 | 사용할 관계 | 현재 코드와 연결할 부분 |
| --- | --- | --- |
| 도착점에서 거리 계산 | 각 노드의 **직접 자식** | `build_children_map()`을 활용해 거리 계산용 BFS에 자식 관계를 제공한다. |
| 경로 존재 여부 확인 | 출발점이 거리 표에 있는지 | 기존의 `start_hash not in distance` 판정을 유지한다. |
| 실제 다음 노드 후보 | 각 노드의 **직접 부모** | 현재 커밋의 `parents`를 읽는다. |
| 최단 경로 후보 제한 | 목표에 도달할 수 있고 거리가 1 줄어드는 부모 | 거리 표에 있는지 먼저 확인한 뒤 거리 값을 비교한다. |
| 동률 선택·복원 | 전체 경로 문자열 비교와 `next_step` | 기존 처리 원리를 유지한다. |

방향 그래프에서는 현재 노드가 목표에 도달할 수 있어도 그 노드의 모든 부모가 목표에 도달할 수 있는 것은 아니다. P, Q, M 세 커밋만 있는 별도 예에서 P와 Q는 각각 부모가 없는 루트이고, M의 부모가 P와 Q라고 하자. `PATH M P`에서 목표 P부터 자식 방향으로 BFS하면 `distance`는 `{'P': 0, 'M': 1}`이다. M의 다른 부모 Q는 표에 없으므로 거리 값을 바로 조회하면 `KeyError`가 발생한다.

따라서 후보 제한은 다음 순서로 설명해야 한다. 아래는 조건 변경을 설명하는 예시이며, 현재 소스에 추가된 코드가 아니다.

```python
for parent_id in commits[current_id].parents:
    if parent_id not in distance:
        continue  # 이 부모에서는 목표에 도달할 수 없다.
    if distance[parent_id] != distance[current_id] - 1:
        continue  # 목표까지 남은 거리가 정확히 1 줄어야 한다.
    # 이 부모의 best_suffix로 후보 문자열을 만들고 기존 최선 후보와 비교한다.
```

거리 표를 BFS 발견 순서대로 처리하면 목표까지 남은 거리가 작은 노드부터 처리한다. 따라서 위 조건을 통과한 부모의 `best_suffix`는 현재 노드보다 먼저 준비된다. 실제 경로 후보만 제한하고, 최단 길이의 동률에서는 여전히 `hash1->hash2->...` 전체 문자열이 가장 작은 경로를 선택한다.

출발점에서 부모 방향으로 BFS하도록 구조를 바꾸는 대안도 있다. 이 경우에는 거리 표의 의미와 경로 복원·동률 처리를 그 구조에 맞춰야 한다. 간선 비용이 모두 1인 점은 같으므로 방향 그래프에서도 BFS를 계속 사용할 수 있다. 방향성이 생겼다는 이유로 DFS로 바꿀 필요는 없다.

**평가 답변 요약:** “부모 방향만 이동하면 경로 존재 여부가 비대칭이 됩니다. 제 코드는 도착점에서 BFS하므로 거리 계산에는 반대 방향인 자식 관계를 사용하고, 실제 경로 선택에는 부모 관계를 사용하겠습니다. 목표에 도달하지 못하는 부모는 제외하고, 남은 거리가 1 줄어드는 후보들 사이에서 기존 사전순 동률 규칙을 적용하겠습니다.”

이는 평가 4-2의 가정에 대한 설명이다. 현재 과제의 무방향 PATH 요구사항이나 구현을 변경하는 지시가 아니다.
...
**문제 18.** PATH를 부모 방향만 허용하도록 바꾸면 B에서 C에 도달할 수 있는가? 현재처럼 도착점에서 BFS하는 구조를 유지한다면 거리 계산과 실제 경로 선택에 각각 어떤 방향의 관계를 사용해야 하는가? BFS 자체를 반드시 다른 알고리즘으로 바꾸어야 하는가?
...
**18.** 부모 방향만 따라가면 B → A에서 끝나므로 C에 도달할 수 없다. 현재처럼 도착점에서 거리를 구한다면 실제 이동을 뒤집은 **자식 방향**으로 BFS하고, 실제 경로는 **부모 방향**으로 선택해야 한다. 부모 후보가 거리 표에 있고 목표까지 남은 거리가 1 줄어드는지 확인한 뒤, 최단 경로 동률 규칙을 적용한다. 거리 계산까지 부모 방향으로 바꾸면 존재하는 경로를 놓칠 수 있다. 간선 비용이 1인 점은 같으므로 BFS는 계속 사용할 수 있다. 자세한 값 추적은 [17.2절](#s17-2)을 참고한다.
